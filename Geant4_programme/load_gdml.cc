...
*/
   ...
   {
     //G4UIExecutive* ui = new G4UIExecutive(argc, argv);   //это убираем для отключения визуалки и терминала
     UImanager->ApplyCommand(looop);//UImanager->ApplyCommand(looop);// UImanager->ApplyCommand("/control/loop g01.mac Ekin 1.0 1.0 1.0");//"/control/execute vis.mac");  //control/loop g01.mac Ekin 1 8 2   looop
    // ui->SessionStart(); //это убираем для отключения визуалки и терминала
   // delete ui; //это убираем для отключения визуалки и терминала
   }
...
/*
 ...
 else           // interactive mode
 {
  // G4UIExecutive* ui = new G4UIExecutive(argc, argv);
   UImanager->ApplyCommand(looop); //control/loop g01.mac Ekin 1 8 2   looop
  // ui->SessionStart();
  // delete ui;
 }
